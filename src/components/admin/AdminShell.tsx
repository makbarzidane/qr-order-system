...
function NavLink({ label, href, icon, active, onClick }: { label: string; href: string; icon: IconName; active?: boolean; onClick?: () => void }) {
  return (
    <Link href={href} onClick={onClick} className={`group flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold transition ${active ? 'bg-amber-500 text-slate-950 shadow-sm' : 'text-slate-400 hover:bg-slate-900 hover:text-white'}`}>
      <span className={`grid h-8 w-8 place-items-center rounded-lg border transition ${active ? 'border-slate-950/10 bg-white/30' : 'border-slate-800 bg-slate-900 text-slate-500 group-hover:border-slate-700 group-hover:text-white'}`}><Icon name={icon} /></span>
      {label}
    </Link>
  )
}
  ...
  const sidebar = (
    <div className="flex h-full flex-col bg-slate-950 text-slate-200">
      <div className="flex h-20 items-center gap-3 border-b border-slate-800 px-5">
        <div className="grid h-11 w-11 place-items-center rounded-2xl bg-amber-500 text-lg font-black text-slate-950">Q</div>
        <div className="min-w-0"><p className="truncate text-sm font-extrabold text-white">{cafeName}</p><p className="text-xs text-slate-500">QR Order CMS</p></div>
      </div>
      <nav className="flex-1 space-y-1 overflow-y-auto px-3 py-5">
        <p className="mb-2 px-3 text-[10px] font-bold uppercase tracking-[0.18em] text-slate-600">Workspace</p>
        {nav.map((item) => {
        ...
        <p className="mb-2 px-3 text-[10px] font-bold uppercase tracking-[0.18em] text-slate-600">Operasional</p>
      ...
      <div className="border-t border-slate-800 p-4">
        ...
        <LogoutButton className="w-full rounded-xl border border-slate-800 px-3 py-2.5 text-left text-xs font-semibold text-slate-400 transition hover:bg-slate-900 hover:text-white" label="Keluar" />
      </div>
    </div>
  )
    ...
    <div className="min-h-screen bg-slate-100 text-slate-900">
      ...
      <div className="lg:pl-64">
        <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-slate-200 bg-white/95 px-4 backdrop-blur sm:px-6 lg:px-8">
          <div className="flex items-center gap-3"><button onClick={() => setOpen(true)} className="rounded-lg border border-slate-200 p-2 text-slate-600 lg:hidden" aria-label="Buka menu"><span className="block h-0.5 w-5 bg-current" /><span className="mt-1.5 block h-0.5 w-5 bg-current" /></button><div><p className="text-base font-extrabold text-slate-950">{currentTitle}</p><p className="hidden text-xs text-slate-500 sm:block">Kelola operasional cafe dari satu tempat</p></div></div>
          <div className="flex items-center gap-2"><span className="hidden items-center gap-2 rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1.5 text-xs font-semibold text-emerald-700 sm:inline-flex"><i className="h-2 w-2 rounded-full bg-emerald-500" />Sistem aktif</span><Link href="/menu/meja-1" target="_blank" className="rounded-lg border border-slate-200 px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50">Lihat storefront</Link></div>
        </header>
